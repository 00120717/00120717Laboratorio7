...
void calculateLocalA(int i,Matrix &A,mesh m){
    ...
    A.at(0).at(0) = calcularTenedor(e,YE,2,1,m); A.at(0).at(1) = calcularTenedor(e,YE,2,1,m);
    A.at(1).at(0) = calcularTenedor(e,EQUIS,3,1,m); A.at(1).at(1) = calcularTenedor(e,EQUIS,3,1,m);
}
...
Matrix createLocalK(int e,mesh &m){
    ...
    float t,k,y,w,d,n,Ae,J,D;
    
    //Componentes de K
    // [ A+K  J-H ]
    // [  C    0  ]
    Matrix matrixA,matrixK,matrixJ,matrixH,matrixC,matrixH_Negativa;
    Matrix K,g_matrix,g_matrix_t,Alpha,Beta,Alphat,Betat,BPrima,BPrimat;

    //Preparando matrixA
    t = m.getParameter(CTE_T);
    ...
    //Preparando matrixJ
    d = m.getParameter(CTE_d);
    calculateBPrima(BPrima);
    productRealMatrix(J*d/(48*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixJ);

    //Preparando matrixH
    y = m.getParameter(CTE_Y);
    calculateBPrima(BPrima);
    productRealMatrix(y*J/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixH);

    //Preparando matrixH_negativa
    productRealMatrix(-1,matrixH,matrixH_Negativa);

    //Preparando matrixC
    transpose(BPrima,BPrimat);
    transpose(g_matrix,g_matrix_t);
    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixC);

    //Colocando submatrices en K
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixJ,matrixH_Negativa,6,3));
    ubicarSubMatriz(K,6,8,0,5,matrixC);
...
Vector createLocalb(int e,mesh &m){
    ...
    float w = m.getParameter(CTE_W);
    float n = m.getParameter(CTE_N);
